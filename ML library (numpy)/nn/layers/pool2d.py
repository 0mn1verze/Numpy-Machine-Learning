...
class Pool2D(Layer):
    ...
    def __to_view(self, X, pool_size, s):
        ...
        out = np.lib.stride_tricks.as_strided(X, out_shape, strides)
        return out
    ...
    def __pooling(self, X, pool_size=(2,2), s=(2,2)):
        ...
        if self.pool_type == "max":
            return np.nanmax(view, axis=(-2, -1))
        elif self.pool_type == "mean":
            return np.nanmean(view, axis=(-2, -1))
        else:
    ...
    def __expand(self, view, dZ, Xp):
        ...
        max_idx = np.argmax(flatten, axis=1) + np.arange(0, view_size , Kh * Kw)

        unravelled = np.unravel_index(max_idx, view.shape)


        dXp_view = self.__to_view(np.zeros_like(Xp), self.pool_size, self.s)

        np.add.at(dXp_view, unravelled, dZ.flatten())

        return np.lib.stride_tricks.as_strided(dXp_view, Xp.shape, Xp.strides)
