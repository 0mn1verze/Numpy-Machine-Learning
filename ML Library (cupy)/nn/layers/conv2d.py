...
class Conv2D(Layer):
    ...
    def __dilate2D(self, X, Dr=(1, 1)):
        dh, dw = Dr
        m, C, H, W = X.shape
        Xd = np.insert(arr=X.get(), obj=np.repeat(np.arange(1, W), dw-1), values=0, axis=-1)
    ...
    def __prepare_subM(self, X, Kh, Kw, s):
        ...
        subM = cp.lib.stride_tricks.as_strided(X, shape=(m, Nc, Oh, Ow, Kh, Kw), strides=strides)
    ...
    def __convolve(self, X, K, s=(1,1), mode='forward'):
        ...
        if mode=='forward':
            return cp.einsum('fckl,mcijkl->mfij', K, subM, optimize=True)
        elif mode=='backward':
            return cp.einsum('fdkl,mcijkl->mdij', K, subM, optimize=True)
        elif mode=='param':
            return cp.einsum('mfkl,mcijkl->fcij', K, subM, optimize=True)
    ...
    def __dZ_D_dX(self, dZ_D, Nh, Nw):
        ...
        K_rotated =  np.rot90(self.K, 2, axes=(-2, -1))
    ...
    def update(self, lr, m, k):
        ...
        if self.regulariser[0].lower()=='l2':
            ...
            dK += self.regulariser[1] * cp.sign(self.K)
