...
class NN:
    ...
    def fit(self, X, y, epochs=10, batch_size=5, lr=1, X_val=None, y_val=None, lr_decay=Constant(), **kwargs):
        ...
        for epoch in range(epochs):
            ...
            accuracy_train = cp.mean(cp.array(y_pred_train) == cp.array(y_train))
    ...
    def evaluate(self, X, y, batch_size=None):
        ...
        for i in tqdm(range(0, len(X), batch_size)):
            ...
            pred, ans = self.cost_func.predictions(Z, y_batch)
            correct += cp.sum(cp.array(pred) == cp.array(ans))
    ...
    def accuracy_plot(self):
        for k, val in self.history.items():
            self.history[k] = cp.array(val).get()
    ...
    def cm_plot(self, X_test, y_test, batch_size=None):
        ...
        cm = np.array(confusion_matrix(y_test.get(), y_pred.get()))

        sns.heatmap(cm, annot=True, fmt="g", xticklabels=cp.arange(10), yticklabels=cp.arange(10))
    ...
    def predict(self, X, batch_size=None): 
        ...
        for i in range(0, len(X), batch_size):
            ...
            y_pred += cp.argmax(Z, axis=1).tolist()
    ...
    def accuracy(self, X_test, y_test, batch_size=None):
        ...
        acc = accuracy_score(y_test.get(), y_pred.get())
