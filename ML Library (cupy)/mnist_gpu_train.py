...
lr, epochs, batch_size = 0.1, 60, 400
in_dim = X_train.shape[1]
out_dim = y_train.shape[1]

model = [
    Dense(240, seed=42, initialiser=Xavier()),
    Tanh(),
    Dense(200, seed=42, initialiser=He()),
    ReLU(),
    Dense(out_dim, seed=42, initialiser=RandNorm()),
    SoftMax()
]
...
nn.fit(X_train, y_train, epochs=epochs, batch_size=batch_size, lr=lr, X_val=X_val, y_val=y_val)

save_model(nn, "mnist.mdl")